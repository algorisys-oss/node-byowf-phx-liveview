[← Step 57: Virtual Fields](57-virtual-fields.md)

# Step 58 — Todo App (Capstone)

## What We're Building

A complete CRUD Todo application with subtasks that brings together every piece of the Blaze framework: LiveView for real-time server-rendered UI, Ember ORM for database operations, PubSub for multi-tab sync, and Tailwind CSS for styling. This is the capstone project demonstrating the full stack.

## Features

### Todos
- **Add todos** with validation (required, max 200 chars)
- **Toggle** completion status (checkbox with visual feedback)
- **Inline edit** — click a todo title to edit in place
- **Delete** individual todos (hover to reveal delete button)
- **Filter** — All / Active / Completed tabs
- **Toggle all** — mark all as complete/incomplete
- **Clear completed** — bulk delete finished todos

### Subtasks
- **Add subtasks** to any todo (hover to reveal "+ sub" button)
- **Toggle subtask** completion (nested checkboxes)
- **Inline edit subtask** — click subtask title to edit in place
- **Delete subtask** (hover to reveal delete button)
- **Expand/collapse** subtask list per todo
- **Progress bar** showing completion percentage per todo
- **Auto-complete parent** when all subtasks are done
- **Auto-uncomplete parent** when a subtask is uncompleted
- **Cascade toggle** — toggling a parent toggles all its subtasks
- **Cascade delete** — deleting a parent deletes all its subtasks

### Cross-cutting
- **Real-time sync** — changes appear instantly in other tabs via PubSub
- **Persistent storage** — SQLite via Ember ORM, survives server restarts
- **Validation errors** — displayed in a red banner when changeset is invalid

## Architecture

```
Browser (WebSocket)
  ↕ bv-click, bv-submit events
TodoLive (LiveView)
  ↕ handleEvent → Changeset → Repo → SQLite
  ↕ PubSub broadcast → handleInfo → re-render
  ↓ render() → bv`` tagged template → diff → client
```
...
subtasks (child)
  ├── id, title, completed, todo_id, position
  └── belongsTo → todos (via todo_id FK)
```
...
| UI | LiveView (server-rendered, WebSocket diffs) |
| Templates | `bv` tagged template literals |
| Styling | Tailwind CSS (CDN) |
| ORM | Ember (Schema, Changeset, Repo, Migrations) |
| Database | SQLite via better-sqlite3 |
| Real-time | PubSub (in-memory topic broadcasts) |

## The Code

### `src/my_app/todo_live.ts`

This is the entire application in a single LiveView file (~610 lines). It includes schemas, migrations, database setup, helper functions, event handling, and the full HTML render.
...
### `src/blaze/server.ts` — Tailwind CDN

The LiveView shell page needs the Tailwind CSS CDN script added to the `<head>`:
...
The `<body>` tag also gets `class="bg-gray-50 min-h-screen"` so Tailwind utility classes work throughout all LiveView pages.
...
liveRoutes.set("/todos", TodoLive);

// Landing page gets a link:
...
```bash
npm run dev
# Open http://localhost:4001/todos
# Open a second tab to see real-time sync
```

1. Add a todo — type a title and press Add
2. Add subtasks — hover a todo, click "+ sub"
3. Toggle subtasks — complete all subtasks, watch parent auto-complete
4. Expand/collapse — click the subtask count badge
5. Edit inline — click any title to edit in place
6. Filter — switch between All / Active / Done tabs
7. Multi-tab — open a second tab, changes sync in real-time

## File Checklist

| File | Status | Description |
|------|--------|-------------|
| `src/my_app/todo_live.ts` | **New** | Full CRUD Todo + Subtask LiveView with Ember ORM |
| `src/blaze/server.ts` | **Modified** | Added Tailwind CSS CDN to LiveView shell |
| `src/app.ts` | **Modified** | Added `/todos` route and landing page link |

## Module 11 Complete!

The Blaze framework is now demonstrated end-to-end with a real application. Every feature from Steps 0-57 comes together:

- **Steps 0-13:** HTTP server, routing, middleware, context, templates
- **Steps 14-21:** uWebSockets.js, LiveView, PubSub, components
- **Steps 22-30:** Streams, uploads, presence, flash, sessions
- **Steps 31-43:** Logger, static assets, testing, health, SSL, clustering
- **Steps 44-57:** Ember ORM (schema, changeset, repo, query, migrations, associations)
- **Step 58:** Todo App -- the capstone that ties it all together
