...
#
# publish-oss.sh — Publish a cleaned copy (without LLM hints) to the OSS repo.
#
# Usage:
#   ./scripts/publish-oss.sh                          # dry-run (default)
#   ./scripts/publish-oss.sh --push                   # actually push to OSS remote
#   OSS_REMOTE=https://github.com/algorisys-oss/node-byowf-phx-liveview.git ./scripts/publish-oss.sh --push
#
...
OSS_REMOTE="${OSS_REMOTE:-https://github.com/algorisys-oss/node-byowf-phx-liveview.git}"
...
if [[ "$DRY_RUN" == false && -z "$OSS_REMOTE" ]]; then
    error "OSS_REMOTE is not set. Export it or pass it inline:\n  OSS_REMOTE=https://github.com/algorisys-oss/node-byowf-phx-liveview.git $0 --push"
fi
...
(cd "$WORK_DIR" && find . -maxdepth 3 -not -path '*/node_modules/*' -not -path '*/dist/*' -not -path './.git/*' | sort)

echo ""
info "package.json project info:"
grep -E '("name"|"version")' "$WORK_DIR/package.json" || true
...
Source: $SOURCE_SHA"
        ...
        COMMIT_MSG="Complete all tutorials with full implementation code
